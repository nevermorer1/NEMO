...
class TestGetGwasResultBB(unittest.TestCase):
    """历史记录-查询基因编辑距离对比计算结果 BB端"""

    @classmethod
    def setUpClass(cls):
        Log.info('TestGetGwasResultBB START BB端')
        cls.node = 2
        cls.dh = DataHandle()
        # 布里端
    ...
    def test_get_Dominant_result_no_session_fail(self):
        """BB 历史记录-查询Dominant显性计算结果 未登录，失败"""
    ...
    def test_get_Recessive_result_no_session_fail(self):
        """BB 历史记录-查询Recessive隐性计算结果 未登录，失败"""
    ...
    def tearDownClass(cls):
        Log.info('TestGetGwasResultBB END BB端')
